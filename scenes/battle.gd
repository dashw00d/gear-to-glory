...
@onready var damage_numbers_layer = $DamageNumberCanvas
...
func on_hit(damage, position):
	spawn_damage_number(damage, position)
...
func spawn_damage_number(value: float, pos: Vector2):
	...
	var screen_pos = get_viewport_transform() * pos
	$DamageNumberCanvas.add_child(damage_number, true)
	damage_number.set_values_and_animate(val, screen_pos, height, spread)
...
func get_damage_number() -> DamageNumber2D:
	...
	# create a new damage number if the pool is empty
		...
		new_damage_number.tree_exiting.connect(func(): damage_number_2d_pool.append(new_damage_number))
