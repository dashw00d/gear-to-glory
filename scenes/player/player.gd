...
@onready var paths = {
	...
	"chest":
	["Skeleton2D/torso/Torso/Armor", "Skeleton2D/torso/Torso/ShoulderL", "Skeleton2D/RShoulder"],
...
func apply_damage(damage: int):
	...
	emit_signal(
		"damage_taken", int(total_damage_taken), %DamageTarget.get_global_transform_with_canvas()
	)
	if health <= 0:
		queue_free()
	...
	for key in current_character.wearable_types:
		...
		if current_character.state["equipment"].has(key):
			...
			if equipment:
				...
				if (
					"cosmetics" in equipment
					and equipment.cosmetics.size() == equipment_paths.size()
				):
