...
func _enter_tree():
	item_editor = preload("res://global/tools/item_editor.tscn").instantiate()
	...
	set_editor = preload("res://global/tools/set_editor.tscn").instantiate()
...
func _on_save_item(item_data: Dictionary, path: String):
	if !path:
		path = "res://assets/data/item_database_test.json"
